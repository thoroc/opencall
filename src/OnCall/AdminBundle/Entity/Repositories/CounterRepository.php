...
class CounterRepository extends EntityRepository
{
    ...
    {
        if( isset( $row['a_duration'] ) ) $duration = $row['a_duration'];
        else $duration = 0;

        return new ItemAggregate(
                $id, $row['a_total'], $row['a_plead'], $row['a_failed'], $duration
        );
    }
    ...
    public function findItemAggregate( AggregateFilter $filter, $child_ids = array() )
    {
        ...
        $multi_ia = array();
        foreach( $res as $row ) $multi_ia[$row['child_id']] = $this->createItemAggregate( $row['child_id'], $row );

        // set uniques
        foreach( $u_res as $u_row ) $multi_ia[$u_row['child_id']]->setUnique( $u_row['a_unique'] );

        // check child ids and create blank aggregate if needed
        foreach( $child_ids as $chid )
        {
            if( isset( $multi_ia[$chid] ) ) continue;
    ...
    protected function offsetHour( $offset, $hour )
    {
        ...
        if( $res_hour >= 24 ) return $res_hour - 24;

        if( $res_hour < 0 ) return $res_hour + 24;
    ...
    public function findChartAggregate( AggregateFilter $filter )
    {
        ...
        if( $filter->isDaily() ) $dql .= ' group by daily';
        else $dql .= ' group by hourly';
        ...
        if( $filter->isDaily() )
        {
            ...
            foreach( $res as $row ) $multi_ia[$row['daily']] = $this->createItemAggregate( 0, $row );

            // make sure all days have a value
            $period = new DatePeriod(
                    $date_from, new DateInterval( 'P1D' ), $date_to
            );
            foreach( $period as $day )
            {
                $day_index = $day->format( 'Y-m-d' );
                if( !isset( $multi_ia[$day_index] ) ) $multi_ia[$day_index] = new ItemAggregate( 0, 0, 0, 0, 0 );
            }
        }
        // hourly
        else
        {
            ...
            for( $i = 0; $i < 24; $i++ )
            {
                if( !isset( $multi_ia[$i] ) ) $multi_ia[$i] = new ItemAggregate( 0, 0, 0, 0, 0 );
            }
        }
        // sort
        ksort( $multi_ia );
    ...
    protected function findSummary( $client_ids, DateTime $date_from )
    {
        ...
        $dql = 'select sum(c.count_total) as a_total, '
                . 'c.client_id as a_id '
                . 'from OnCallAdminBundle:Counter c '
                . 'where c.date_in >= :date_from and c.date_in <= :date_to '
                . 'and c.client_id in (:client_ids) '
        ...
        // initialize summary
        $summary = array();
        foreach( $client_ids as $cid ) $summary[$cid] = 0;

        // set from result
        foreach( $res as $row ) $summary[$row['a_id']] = $row['a_total'];
