...
namespace OnCall\PlivoBundle\AccountCounter;
...
class Repository
{
    ...
    protected function getCallCountAndDuration( DateTime $date, $user_id )
    {
        ...
        if( !$stmt->execute() ) return array( 'call' => 0, 'duration' => 0 );

        $row = $stmt->fetch();
        if( !$row ) return array( 'call' => 0, 'duration' => 0 );

        if( $row['agg_call'] == null ) $row['agg_call'] = 0;
        if( $row['agg_dur'] == null ) $row['agg_dur'] = 0;
